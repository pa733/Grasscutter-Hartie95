...
public final class SDKHandler implements Router {
    @Override public void applyRoutes(Javalin javalin) {
        ...
        javalin.post("/hk4e_global/mdk/shield/api/login", SDKHandler::clientLogin);
        // Cached token login (from registry).
        javalin.post("/hk4e_global/mdk/shield/api/verify", SDKHandler::tokenLogin);
        // Combo token login (from session key).
        javalin.post("/hk4e_global/combo/granter/login/v2/login", SDKHandler::sessionKeyLogin);
